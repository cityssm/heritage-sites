/* eslint-disable n/no-unpublished-import, n/no-unsupported-features/node-builtins */

import type { BulmaJS } from '@cityssm/bulma-js/types.js'
import type * as Leaflet from 'leaflet'
import type { HeritageSite } from '../data/types.js'
...
;(() => {
  ...
  const closeHeritageSiteModalAndClearHash = () => {
    ...
    let focusTargetElement: HTMLElement = searchResultsElement.querySelector(
      "a[href='" + currentHash + "']"
    ...
    if (heritageSite.hasPage === 'TRUE') {
      ...
      window
        ...
        .then((pageHTML) => {
          ...
          const pageAnchorElements = modalElement.querySelectorAll(
            "[data-field='page'] a"
  ...
  const searchResultsTableInnerHTML =
    '<thead><tr>' +
  ...
  const refreshSearchResults = () => {
    ...
    for (const heritageSite of heritageSites) {
      ...
      if (includeSite) {
        ...
        const descriptionOfSiteAnchorElement = rowElement.querySelector(
          "[data-field='descriptionOfSite']"
        ) as HTMLAnchorElement
        ...
        ;(
          rowElement.querySelector(
            "[data-field='designatedOrListed']"
          ) as HTMLElement
        ).textContent = heritageSite.designatedOrListed
  ...
  if (window.location.search !== '') {
    const searchString = new URL(window.location.href).searchParams.get(
      'searchString'
    )
    if (searchString) {
  ...
  document.addEventListener('keydown', (keyboardEvent) => {
    if (
      modalElement.classList.contains('is-active') &&
